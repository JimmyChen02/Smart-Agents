...
First, clear stale handoff files: empty the .pipeline/ directory (create it if it does not exist) so no agent reads leftovers from a previous run.
Then execute these stages in order. Do not skip ahead. After each stage, confirm the handoff file exists before starting the next.
